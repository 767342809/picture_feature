...
def image_to_tfrecord(bucket, img_file, label, image_id, local_tf, is_save_show_picture=False):
    with tf.Session():
        ...
        bucket.put_object(local_tf, example.SerializeToString())
...
def prepare_train_data_in_oss():
    ...
    for index, row in df.iterrows():
        ...
        try:
            image_to_tfrecord(bucket, img_url, label, img_id, oss_tf)
...
def check_tfrecord():
    ...
    print(prefix)
    for b in islice(oss2.ObjectIterator(bucket, prefix=prefix), 10):
        print(b.key)
        # cnt = cnt + 1
